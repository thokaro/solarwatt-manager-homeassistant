...
_ENERGY_OVERVIEW_ITEM_LABELS = {
    ...
    "gridPower": "Grid Balance",
    "batteryPower": "Battery Balance",
    "selfConsumedPower": "Consumption Direct Consumption",
    "householdFromBatteryPower": "Consumption From Battery",
    "householdFromGridPower": "Consumption From Grid",
    "householdFromPvPower": "Consumption From PV",
...
def energy_overview_to_items(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for item_name in _ENERGY_OVERVIEW_ITEM_NAMES:
        if item_name not in values:
            continue
        label = _ENERGY_OVERVIEW_ITEM_LABELS[item_name]
...
def item_names_to_thing_uids(
    ...
    """Map generated HEMS item names back to their owning things."""
    ...
    for raw_thing in things:
        ...
        if not thing_uid:
            continue
        if slug := _thing_uid_slug(thing_uid):
            thing_uids_by_slug[slug] = thing_uid
...
def hems_configurator_to_things(payload: Sequence[Any]) -> list[dict[str, Any]]:
    """Convert HEMS configurator records to the internal thing shape."""
...
def _thing_uid_slug(thing_id: str) -> str:
